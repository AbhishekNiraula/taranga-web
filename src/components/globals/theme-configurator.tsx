...
const ThemeConfigurator = () => {
	...
	useEffect(() => {
		...
		const initialColors = {
			...
			'card-foreground': parseHSLFromVar(
				root.getPropertyValue('--card-foreground')
			),
			popover: parseHSLFromVar(root.getPropertyValue('--popover')),
			'popover-foreground': parseHSLFromVar(
				root.getPropertyValue('--popover-foreground')
			),
			primary: parseHSLFromVar(root.getPropertyValue('--primary')),
			'primary-foreground': parseHSLFromVar(
				root.getPropertyValue('--primary-foreground')
			),
			secondary: parseHSLFromVar(root.getPropertyValue('--secondary')),
			'secondary-foreground': parseHSLFromVar(
				root.getPropertyValue('--secondary-foreground')
			),
			muted: parseHSLFromVar(root.getPropertyValue('--muted')),
			'muted-foreground': parseHSLFromVar(
				root.getPropertyValue('--muted-foreground')
			),
			accent: parseHSLFromVar(root.getPropertyValue('--accent')),
			'accent-foreground': parseHSLFromVar(
				root.getPropertyValue('--accent-foreground')
			),
			destructive: parseHSLFromVar(root.getPropertyValue('--destructive')),
			'destructive-foreground': parseHSLFromVar(
				root.getPropertyValue('--destructive-foreground')
			),
			...
			<div className="space-y-3 max-h-[70vh] overflow-y-auto">
				{Object.entries(colors).map(([variable, color]) => (
					<div
						...
						<label className="text-sm text-muted-foreground flex-grow">
							{variable}
						</label>
						<button
